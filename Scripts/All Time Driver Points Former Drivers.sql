SELECT
    ...
    rLastRace.last_race AS 'Last Race in F1',
    IF(Year(rLastRace.last_race) <> 2022, 'Not Present in 2022 Season', 'Current Driver') As 'Driver Status'
FROM drivers AS d
...
SELECT 
	r.driverId,
    MAX(rr.raceDate) AS last_race
    FROM results as r
    LEFT JOIN races as rr
    ON r.raceId = rr.raceId
    GROUP BY r.driverId
    ) AS rLastRace
ON d.driverId = rLastRace.driverId

WHERE YEAR(last_race) <> 2022 
